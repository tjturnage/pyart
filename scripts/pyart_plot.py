...
def extract_sweeps(orig_list, cut):
    ...
    """
    # multiply elevations by 10 and round to ensure that possible
    # values listed above are represented. This steep is needed because
    # the elevations are floats and may vary exact.
    # For example, 0.51... and 0.49... are both rounded to 5
    sweep_list = [int(round(x*10)) for x in orig_list]
    cut_list = []
    found_cut = False
    for i, sweep in enumerate(sweep_list):
        if sweep == cut and found_cut is False:
            cut_list.append(i)
            found_cut = True
        elif sweep == cut and found_cut is True:
            found_cut = False
    return cut_list
